...
#find command uses find4tc function
#leaderboard/topten command uses get_leaderboard function
#submissions uses get_submission
#submit function uses the submit function
...
def validate_towers(towers):
    ...
    validTowers = ['Quincy', 'Gwen', 'Striker', 'Obyn', 'Church', 'Ben', 'Ezili', 'Pat', 'Adora', 'Brick', 'Etienne', 'Dart', 'Boomer', 'Bomb', 'Tack', 'Ice', 'Glue', 'Sniper', 'Sub', 'Bucc', 'Ace', 'Heli', 'Mortar', 'Dartling', 'Wizard', 'Super', 'Ninja', 'Alch', 'Druid', 'Spac', 'Village', 'Engi']
...
#takes towers list/tuple with 0-4, towers can be unordered, returns found 4tc formatted for easy reading
def find4tc(towers):
    if not validate_towers(towers):
        return Exception('Invalid towers')

    file = open('C:/Users/roeal/source/repos/4tc_Bot/remaining_combos.json')
...
def get_leaderboard(name=None, amount=None):
    file = open('C:/Users/roeal/source/repos/4tc_Bot/leaderboard.json')
    ...
    displayStr = ''
    if amount == None or amount >= len(leaderboard):
        for score in leaderboard:
...
def get_submissions(name=None, towers=None):
    file = open('C:/Users/roeal/source/repos/4tc_Bot/submitted_combos.json')
    submittedCombos = json.load(file)
    file.close()

    if towers != None:
        for submission in submittedCombos:
            #is_subset is used instead of == since towers can be unordered
            if is_subset(submission[2], towers):
                return 'Name: {0}\nCode: {1}\nTowers: {2}'.format(submission[0], submission[1], ', '.join(submission[2]))
        return Exception('Not submitted')
    ...
    if name == None:
        ...
        if displayStr == '':
            return Exception("Name doesn't exist")

        displayStr = name + "'s submissions\n" + displayStr
...
def change_name(old, new):
    ...
    for score in leaderboard:
        if score[0] == new:
            return Exception('Cannot set alias to another persons name')
        
    for score in leaderboard:
        if score[0] == old:
            score[0] = new

    file = open('C:/Users/roeal/source/repos/4tc_Bot/leaderboard.json', 'w')
    ...
    file = open('C:/Users/roeal/source/repos/4tc_Bot/submitted_combos.json', 'w')
...
def set_alias(discordId, name):
    ...
    file = open('C:/Users/roeal/source/repos/4tc_Bot/aliases.json')
    ...
    if alias == None:
        ...
        try:
            ...
            return e
        displayStr = '{0} updated to {1}'.format(alias, name)
    
    aliases[discordId] = name

    file = open('C:/Users/roeal/source/repos/4tc_Bot/aliases.json', 'w')
...
def get_alias(discordId):
    ...
    aliases = json.load(file)
    file.close()
    return aliases.get(discordId, Exception("Name not provided and you have no alias set"))
...
def get_challenge_data(code):
    try:
        ...
        return Exception('Invalid challenge code')
    except (requests.exceptions.RequestException):
        return Exception("Couldn't establish a connection with nk servers")
...
def get_version(cD):
    ...
    if cD['towers'][11]['tower'] != 'Etienne':
        return 19 #19.0 - 20.1
    #uniqueDCId can't be used to determine versions as it doesn't seem to be always used
...
def get_towers(challengeData):
    ...
    namesNK = ('Quincy', 'Gwendolin', 'StrikerJones', 'ObynGreenfoot', 'CaptainChurchill', 'Benjamin', 'Ezili', 'PatFusty', 'Adora', 'AdmiralBrickell', 'Etienne', 'DartMonkey', 'BoomerangMonkey', 'BombShooter', 'TackShooter', 'IceMonkey', 'GlueGunner', 'SniperMonkey', 'MonkeySub', 'MonkeyBuccaneer', 'MonkeyAce', 'HeliPilot', 'MortarMonkey', 'DartlingGunner', 'WizardMonkey', 'SuperMonkey', 'NinjaMonkey', 'Alchemist', 'Druid', 'SpikeFactory', 'MonkeyVillage', 'EngineerMonkey')
    names4tc = ('Quincy', 'Gwen', 'Striker', 'Obyn', 'Church', 'Ben', 'Ezili', 'Pat', 'Adora', 'Brick', 'Etienne', 'Dart', 'Boomer', 'Bomb', 'Tack', 'Ice', 'Glue', 'Sniper', 'Sub', 'Bucc', 'Ace', 'Heli', 'Mortar', 'Dartling', 'Wizard', 'Super', 'Ninja', 'Alch', 'Druid', 'Spac', 'Village', 'Engi')
...
def remove4tc(towers):
    ...
    if matches == 0:
        file.close()
        return Exception('Already submitted')

    file = open('C:/Users/roeal/source/repos/4tc_Bot/remaining_combos.json', 'w')
